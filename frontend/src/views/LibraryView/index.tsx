    ...
    <div className="animate-fade-in space-y-4 pb-32 relative">
      ...
      {showFilters && (
        <>
          ...
          />
          <div className={`absolute left-4 right-4 md:left-auto md:right-8 md:w-[500px] z-20 p-4 rounded-xl border shadow-xl space-y-4 ${darkMode ? 'bg-stone-800 border-stone-700' : 'bg-white border-zinc-200'}`}>
            <div className="flex items-center justify-between mb-2">
              <span className={`text-sm font-semibold ${darkMode ? 'text-stone-200' : 'text-zinc-700'}`}>{t('library.filters')}</span>
              <button
                onClick={() => setShowFilters(false)}
                className={`p-1 rounded-lg transition-colors ${darkMode ? 'hover:bg-stone-700 text-stone-400' : 'hover:bg-zinc-100 text-zinc-400'}`}
              >
                <Icons.X />
              </button>
            </div>
            {/* Status multi-select */}
            <div className="mb-4">
              <label className={`block text-xs font-medium uppercase tracking-wider mb-2 ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{t('library.status')}</label>
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                      statusFilters.has(status)
                        ? status === 'favorite' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400 ring-2 ring-yellow-400/50' :
                          ...
                          'bg-zinc-100 text-zinc-700 dark:bg-stone-700 dark:text-stone-300 ring-2 ring-zinc-400/50'
                        : darkMode ? 'bg-stone-700 text-stone-400 hover:text-stone-200' : 'bg-zinc-100 text-zinc-500 hover:text-zinc-700'
                    }`}
                  >
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className={`block text-xs font-medium uppercase tracking-wider mb-1.5 ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{t('library.source')}</label>
                <select
                  value={categoryFilter}
                  onChange={(e) => setCategoryFilter(e.target.value)}
                  className={`w-full p-2 rounded-lg border text-sm ${darkMode ? 'bg-stone-900 border-stone-600 text-white' : 'bg-white border-zinc-200'}`}
                >
              ...
              <div>
                <label className={`block text-xs font-medium uppercase tracking-wider mb-1.5 ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{t('library.year')}</label>
                <select
                  value={yearFilter}
              ...
              <div>
                <label className={`block text-xs font-medium uppercase tracking-wider mb-1.5 ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{t('library.firstLetter')}</label>
                <select
                  value={letterFilter}
            ...
            {hasActiveFilters && (
              <button
                onClick={clearAllFilters}
                className={`text-sm font-medium ${darkMode ? 'text-teal-400 hover:text-teal-300' : 'text-spira-600 hover:text-spira-500'}`}
              >
      ...
      {paginatedArticles.length > 0 && (
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-2">
            {selectedIds.size > 0 && (
              <span className={`text-sm font-medium ${darkMode ? 'text-teal-400' : 'text-spira-600'}`}>{selectedIds.size} {t('library.selected')}</span>
            )}
          </div>
          <div className="flex items-center gap-3 cursor-pointer" onClick={selectAll}>
            <span className={`text-xs uppercase tracking-wider font-bold ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{t('library.selectAll')}</span>
            <div
              className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedIds.size === paginatedArticles.length && paginatedArticles.length > 0 ? (darkMode ? 'bg-teal-600 border-teal-600' : 'bg-spira-600 border-spira-600') : (darkMode ? 'border-stone-600' : 'border-zinc-300')}`}
            >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 opacity-50 flex flex-col items-center">
          <div className="mb-4 p-4 bg-zinc-100 rounded-full dark:bg-stone-800"><Icons.Library /></div>
            ...
            return (
              <div
                ...
                className={`relative group flex items-start gap-4 p-4 rounded-xl border cursor-pointer transition-micro active:scale-[0.99] ${
                  isSelected
                    ? (darkMode ? 'bg-teal-900/20 border-teal-500/50' : 'bg-spira-50 border-spira-200')
                    : (darkMode ? 'bg-stone-800 border-stone-700 hover:border-teal-500/50 hover:shadow-lg hover:shadow-teal-900/20 hover:-translate-y-0.5' : 'bg-white border-zinc-200 hover:border-spira-400 hover:shadow-lg hover:shadow-spira-100 hover:-translate-y-0.5')
                }`}
              >
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${darkMode ? 'bg-stone-700 text-teal-300' : 'bg-zinc-100 text-spira-700'}`}>{article.source}</span>
                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                        ...
                        'bg-zinc-100 text-zinc-600 dark:bg-stone-700 dark:text-stone-400'
                      ...
                      <span className={`text-xs ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{article.timestamp}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      {article._entry?.link?.includes('arxiv.org') && article._entry?.ai_content_type === 'error' && (
                        <button
                          ...
                          className={`p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity ${darkMode ? 'hover:bg-stone-700 text-orange-400' : 'hover:bg-zinc-100 text-orange-500'}`}
                      ...
                      {article._entry?.link && (
                        <button
                          ...
                          className={`p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity ${darkMode ? 'hover:bg-stone-700 text-stone-400' : 'hover:bg-zinc-100 text-zinc-400'}`}
                          title="Open original"
                        >
                          <Icons.ExternalLink />
                        </button>
                  ...
                  <h3 className="mb-1">
                    <span
                      ...
                      className={`text-base font-bold cursor-pointer hover:underline decoration-2 underline-offset-2 ${darkMode ? 'text-stone-100 decoration-teal-500' : 'text-zinc-900 decoration-spira-300'}`}
                    >
                      {article.title}
                    </span>
                  </h3>

                  <p className={`text-sm line-clamp-2 mb-2 leading-relaxed ${darkMode ? 'text-stone-400' : 'text-zinc-500'}`}>{article.snippet}</p>

                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
                      {article.tags.slice(0, 3).map(tag => (
                        <span key={tag} className={`text-[10px] px-2 py-0.5 rounded font-medium ${darkMode ? 'bg-stone-700 text-stone-300' : 'bg-zinc-100 text-zinc-600'}`}>#{tag}</span>
                      ))}
                    </div>
                    <span className={`text-xs ${darkMode ? 'text-stone-500' : 'text-zinc-400'}`}>{article.readTime}</span>
                  </div>
                </div>

                <div
                  onClick={(e) => toggleSelect(article.id, e)}
                  className={`mt-1 flex-shrink-0 w-5 h-5 rounded border flex items-center justify-center transition-colors z-10 ${isSelected ? (darkMode ? 'bg-teal-600 border-teal-600' : 'bg-spira-600 border-spira-600') : (darkMode ? 'border-stone-600 hover:border-teal-400' : 'border-zinc-300 hover:border-spira-400')}`}
                >
